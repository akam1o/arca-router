...
For automation against the Web/NMS API, provide a `0600` token file with one `name:role:token` or `name:role:sha256:<hex>` entry per line and start the daemon with `--web-api-token-file=/etc/arca-router/web-api-tokens`. Plain token values must be at least 32 characters, must not contain whitespace, and should be generated from random bytes, for example `openssl rand -base64 32`. Prefer storing `sha256:<hex>` token hashes in the file; clients still present the original token through `Authorization: Bearer <token>` or `X-API-Key: <token>` and reuse the `read-only`, `operator`, and `admin` RBAC roles.
