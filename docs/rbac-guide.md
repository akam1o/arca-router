...
The Web UI uses HTTP Basic authentication when password-backed `security users` exist in the running configuration. All built-in roles can read the dashboard, `/api/status`, and `/api/config`; configuration writes remain unavailable through the Web UI in v0.6.
