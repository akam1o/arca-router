...
- `arca-routerd --web-api-token-file=<path>` enables Web/NMS API automation tokens. The token file must be restricted to `0600`. The file format is one `name:role:token` or `name:role:sha256:<hex>` entry per line, where `role` is `read-only`, `operator`, or `admin`. Plain token values must be at least 32 characters, must not contain whitespace, and should be generated from random bytes, for example `openssl rand -base64 32`. Prefer `sha256:<hex>` entries for stored files; requests still use the original bearer token through `Authorization: Bearer <token>` or `X-API-Key: <token>`.
