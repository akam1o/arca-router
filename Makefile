...
rpm-verify: rpm ## Verify RPM package reproducibility
	@echo "Verifying reproducible build..."
	@rm -f $(DIST_DIR)/arca-router-*.rpm.1
	@mv $(DIST_DIR)/arca-router-*.rpm $(DIST_DIR)/arca-router-$(VERSION)-1.x86_64.rpm.1
	@$(MAKE) rpm
	@echo "Comparing checksums..."
	@sha256sum $(DIST_DIR)/arca-router-*.rpm $(DIST_DIR)/arca-router-*.rpm.1
